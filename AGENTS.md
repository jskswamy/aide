...
**When ending a work session**, you MUST complete ALL steps below. Work is NOT complete until `git push` of your feature branch succeeds. Pushing the branch is not the same as merging it - per CONTRIBUTING.md, changes land on `main` only via a PR that passes CI and the two-stage (AI + human) review, not by pushing directly to `main`.
...
4. **PUSH YOUR FEATURE BRANCH TO REMOTE** - This is MANDATORY:
   ```bash
   git pull --rebase
   bd dolt push
   git push
   ...
   ```
5. **Open or update the PR** - see CONTRIBUTING.md for the review workflow; work isn't merged until it passes review
6. **Clean up** - Clear stashes, prune remote branches
7. **Verify** - All changes committed AND pushed
8. **Hand off** - Provide context for next session

**CRITICAL RULES:**
- Work is NOT complete until your feature branch is pushed
- NEVER stop before pushing - that leaves work stranded locally
- NEVER say "ready to push when you are" - YOU must push
- If push fails, resolve and retry until it succeeds
- Pushing a branch is not a merge - main only changes via reviewed PR (see CONTRIBUTING.md)
